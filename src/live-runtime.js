...
if (payloadElement) {
  ...
  function parseMetadata(header) {
    const tokens = header.match(/(?:[^\s{]+|static:\{[^}]*\})+/g) || [];
    return {
      live: tokens.some((token) => token.toLowerCase() === "live"),
  ...
  function makeRuntime(sliders) {
    ...
    const runtime = { registry, context: new Context(), systemContext: null, currentCell: 0, sliderCount: 0 };
    ...
    systemContext.register("SLIDER", {
      impl(args) {
        ...
        sliders.push({ ...config, id, index, value });
        return value;
      },
  ...
  function run() {
    ...
    for (const cell of cells) {
      const results = executeCell(cell, runtime);
      if (cell.metadata.live) renderWidget(cell, results);
    }
    renderControls(sliders);
  }

  function renderWidget(cell, results) {
    ...
    if (cell.metadata.showOutput && result) {
      output = result.error
        ...
        : `<div class="rix-live-output">${isOutputValue(result.value) ? renderOutputHtml(result.value, formatValue) : `<pre>${escapeLiveHtml(formatValue(result.value))}</pre>`}</div>`;
    }
